...
class SUVRClassificationLoss(nn.Module):
    ...
    def forward(self, pred_logits, gt_state, aal_label):
        ...
        targets = torch.full_like(gt_state, fill_value=self.ignore_index) 
        cls_mask = valid_mask & (gt_state >= 1) & (gt_state <= 3) 
...
class PETFoundationModel(nn.Module):
    def __init__(
        ...
        text_pack_path="aal_text_pack.pt",
        num_aal=170,
        text_dim=512,
        proj_dim=512,
    ...
    def forward(
        ...
        if (aal_label is not None) and (aal_state is not None) and (weight_clip > 0):
            ...
            weights_aal = weights_aal / (weights_aal.sum(dim=1, keepdim=True) + 1e-8)
